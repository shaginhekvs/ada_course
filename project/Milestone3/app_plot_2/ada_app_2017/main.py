...
class Create_image_html():
    ...
    def __init__(self,image_path,text,width=900):
        #self.space=Paragraph(text='')
        self.para=Div(text='<h2 style="color:#3b3a39;text-align:center;font-size:150%;font-family:Times New Roman;">{}</h1>'.format(text),width=width)
        
        self.div=Div(text='<img src="{}" alt="{}" border="0" height="300" width="{}"></img>'.format(image_path,text,width),width=width,height=500)
        self.width=width
        self.div.css_classes = ["custom"]
    def return_view(self):
        return column(self.para,self.div)
    ...
    def reinit(self,image_path,text):
        ...
        self.div.text='<img src="{}" alt="{}" border="0" height="300" width="{}"></img>'.format(image_path,text,self.width)
        
        
class Create_text_html():
    
    def __init__(self,image_path,text,width=900):
        #self.space=Paragraph(text='')
        self.para=Div(text='<h2 style="color:#3b3a39;text-align:center;font-size:150%;font-family:"helvetica";">{}</h1>'.format(text),width=width)
...
def update_source(attrname, old, new):
    ...
    if(new=='Trump'):
        ...
        cloud_antimaga.reinit(antimaga_img,'')
...
blm_img="/ada_app_2017/static/images/blm.png"
antimaga_img="/ada_app_2017/static/images/resist_cloud.png"
miso_img= "/ada_app_2017/static/images/MISOGYNY.png"
...
percent_subjectivity_plot_miso=Create_percent_sentiment_plot( join(data_path_prefix,'subjectivity_percent_df_miso'),'Subjectivity',\
                                                        'Misogyny' ,join(data_path_prefix,'events_isabela'),width=1600)
...
cloud_miso=Create_image_html(miso_img,'',1200)

cloud_antimaga=Create_image_html(antimaga_img,'',1200)
...
tab_blm = Panel(child=column(widgetbox(select_data_source),time_count_plot1.return_view()\
                             ,net_row,time_polarity_plot1.return_view(),time_subjectivity_plot1.return_view(),percent_subj_row,percent_polar_row),title='Comparisons')
